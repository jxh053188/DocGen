var BundledCode = (() => {
    ...
    // http-url:https://unpkg.com/html2canvas@1.4.1/dist/html2canvas.js
    var require_html2canvas = __commonJS({
        "http-url:https://unpkg.com/html2canvas@1.4.1/dist/html2canvas.js"(exports, module) {
    ...
    __export(input_exports, {
        default: () => import_html2canvas_1_4.default
    });
    __reExport(input_exports, __toESM(require_html2canvas()));
    var import_html2canvas_1_4 = __toESM(require_html2canvas());
...
// This allows the library to be accessed as window.html2canvas after loading via loadScript
(function() {
    if (typeof window !== 'undefined') {
        ...
        const assignToWindow = () => {
            attempts++;
            try {
                // html2canvas is wrapped in a CommonJS module loader
                // The UMD pattern assigns to module.exports, but in browser context it should also assign to global
                // However, since it's wrapped, we need to check multiple places
                let html2canvasFunc = null;
                
                // Check if BundledCode (the IIFE result) is available and contains html2canvas
                // BundledCode is the result of __toCommonJS(input_exports), which should have a default property
                if (typeof BundledCode !== 'undefined' && BundledCode) {
                    try {
                        // Try default property first (ESM export)
                        if (BundledCode.default && typeof BundledCode.default === 'function') {
                            html2canvasFunc = BundledCode.default;
                        } 
                        // If BundledCode itself is a function, use it
                        else if (typeof BundledCode === 'function') {
                            html2canvasFunc = BundledCode;
                        }
                        // Check if it's an object with the function as a property
                        else if (BundledCode.html2canvas && typeof BundledCode.html2canvas === 'function') {
                            html2canvasFunc = BundledCode.html2canvas;
                        }
                    } catch (e) {
                        // BundledCode might not be ready yet
                    }
                }
                
                // Check global scopes (globalThis, self, window)
                if (!html2canvasFunc) {
                    const scopes = [
                    ...
                    for (const scope of scopes) {
                        if (scope && scope.html2canvas && typeof scope.html2canvas === 'function') {
                            html2canvasFunc = scope.html2canvas;
                            break;
                        }
                    }
                }
                
                if (html2canvasFunc && typeof html2canvasFunc === 'function') {
                    window.html2canvas = html2canvasFunc;
                    console.log('✅ html2canvas static resource assigned to window.html2canvas, type:', typeof window.html2canvas);
                    return true;
                } else if (attempts < maxAttempts) {
                    // Retry after a short delay
                    setTimeout(assignToWindow, checkInterval);
                    return false;
                } else {
                    console.warn('⚠️ html2canvas: html2canvas not found after', maxAttempts, 'attempts.');
                    console.warn('⚠️ html2canvas: BundledCode type:', typeof BundledCode);
                    console.warn('⚠️ html2canvas: globalThis.html2canvas type:', typeof (globalThis?.html2canvas || undefined));
                    console.warn('⚠️ html2canvas: self.html2canvas type:', typeof (self?.html2canvas || undefined));
                    console.warn('⚠️ html2canvas: window.html2canvas type:', typeof (window.html2canvas || undefined));
                    // Last resort: try to manually extract from the UMD pattern by checking if the factory executed
                    // The UMD should have assigned to globalThis or self
                    return false;
                }
            } catch (e) {
                if (attempts < maxAttempts) {
                    ...
                    console.error('❌ html2canvas: Error assigning html2canvas to window:', e);
                    console.error('❌ html2canvas: Error stack:', e?.stack);
                    return false;
                }
            }
        };
        ...
        console.warn('html2canvas: window is not defined. Cannot assign to window.html2canvas.');
    }
})();
